    ...
    : QAbstractItemModel {parent}
{
    ...
    const auto *rssManager = RSS::Manager::instance();
    connect(rssManager, &RSS::Manager::itemAdded, this, &RSSModel::itemAdded);
    connect(rssManager, &RSS::Manager::itemPathChanged, this, &RSSModel::itemPathChanged);
    connect(rssManager, &RSS::Manager::itemAboutToBeRemoved, this, &RSSModel::itemAboutToBeRemoved);
    connect(rssManager, &RSS::Manager::feedStateChanged, this, &RSSModel::feedStateChanged);
}
...
int RSSModel::rowCount(const QModelIndex &parent) const
{
    ...
    if (!parent.isValid())
        return RSS::Manager::instance()->rootFolder()->items().count() + 1;
...
{
    // "Unread" item
    connect(RSS::Manager::instance()->rootFolder(), &RSS::Item::unreadCountChanged, this, &RSSModel::itemUnreadCountChanged);

    populate(RSS::Manager::instance()->rootFolder());
}
...
{
    const auto parentIndex = index(rssItem->parent());
...
void RSSModel::feedIconDownloadFinished(RSS::Feed *feed, const QString &filePath)
{
    ...
    const QString iconPath = Utils::Fs::fromNativePath(filePath);
...
void RSSModel::addItem(RSS::Item *rssItem)
{
    if (auto *feed = qobject_cast<RSS::Feed *>(rssItem)) {
        ...
        connect(handler
                ...
                , feed, [this, feed](const QString &, const QString &filePath)
        {
            feedIconDownloadFinished(feed, filePath);
        });
    }
