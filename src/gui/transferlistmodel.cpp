...
QString TransferListModel::displayValue(const BitTorrent::TorrentHandle *torrent, int column) const
{
    ...
    const auto availabilityString = [hideValues](const qreal value)
    {
        return ((value <= 0) && hideValues)
                ? QString {} : Utils::String::fromDouble(value, 3);
    };

    const auto unitString = [hideValues](const qint64 value, const bool isSpeedUnit = false)
    {
        return ((value == 0) && hideValues)
                ? QString {} : Utils::Misc::friendlyUnit(value, isSpeedUnit);
    };

    const auto limitString = [hideValues](const qint64 value)
    {
        return ((value == 0) && hideValues)
                ? QString {}
                : (value > 0)
    ...
    const auto amountString = [hideValues](const qint64 value, const qint64 total)
    {
        return ((value == 0) && (total == 0) && hideValues)
                ? QString {} : QString::number(value) + " (" + QString::number(total) + ')';
    };

    const auto ratioString = [hideValues](const qreal value)
    {
        return ((value <= 0) && hideValues)
                ? QString {}
                : ((static_cast<int>(value) == -1) || (value > BitTorrent::TorrentHandle::MAX_RATIO))
                  ? QString::fromUtf8(C_INFINITY) : Utils::String::fromDouble(value, 2);
    };

    const auto queuePositionString = [](const qint64 value)
    {
        return (value > 0) ? QString::number(value) : "*";
    };

    const auto lastActivityString = [hideValues](qint64 value)
    {
    ...
    const auto timeElapsedString = [](const qint64 elapsedTime, const qint64 seedingTime)
    {
        return (seedingTime > 0)
    ...
    const auto tagsString = [](const QSet<QString> &tags)
    {
    ...
    const auto progressString = [](qreal progress)
    {
        progress *= 100;
    ...
    case TR_DLSPEED:
        return unitString(torrent->downloadPayloadRate());
    case TR_UPSPEED:
        return unitString(torrent->uploadPayloadRate());
    case TR_ETA:
...
QVariant TransferListModel::internalValue(const BitTorrent::TorrentHandle *torrent, int column, bool alt) const
{
    switch (column) {
    case TR_NAME:
        return torrent->name();
...
QString TransferListModel::statusString(const BitTorrent::TorrentState state, const QString &errorMessage) const
{
